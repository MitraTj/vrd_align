...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, multiple_preds=False, spice=False):
        self.result_dict = {}
        self.mode = mode
        ...
        self.spice=spice
        if self.spice:
            self.result_dict[self.mode + '_spice'] = {20: [], 50: [], 100: []}
    ...
    def print_stats(self):
        ...
        if self.spice:
            for k, v in self.result_dict[self.mode + '_spice'].items():
                print('SPICE@%i: %f' %(k, np.mean(v)))

def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, multiple_preds=False, spice=False,
    ...
    for k in result_dict[mode + '_recall']:
        ...
        if spice:
            pre_i = float(len(match)) / k
            f1_i = 2 * pre_i * rec_i / (pre_i + rec_i + 1e-8)
            result_dict[mode + '_spice'][k].append(f1_i)
