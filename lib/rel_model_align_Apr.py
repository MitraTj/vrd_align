...
##            import pdb; pdb.set_trace()
            y1 = self.gumbel_softmax(SO_fmaps_logits, temperature, training=False)
            #SO_fmaps_scores = F.softmax(y, dim=1)
           
            weighted_S_fmaps = torch.matmul(y1.transpose(2, 1), S_fmaps_trans) # (num_rels, 49, 49) x (num_rels, 49, self.reduce_dim)
