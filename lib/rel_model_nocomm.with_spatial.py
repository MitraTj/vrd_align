...
class SceneDynamicContext(nn.Module):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', use_vision=True,
        ...
        self.rel_compress = nn.Linear(self.hidden_dim*3 + 128, self.num_rels, bias=True)
        ...
        # Spatial encoding
        self.spat2feat = torch.nn.Linear(28, 128)
    ...
    # follow NIPSW18 spatial feature
    def relative_spatial_encode(self, obj1, obj2):
        obj1_x1, obj1_y1, obj1_x2, obj1_y2 = obj1[:, 0], obj1[:, 1], obj1[:, 2], obj1[:, 3]
        obj2_x1, obj2_y1, obj2_x2, obj2_y2 = obj2[:, 0], obj2[:, 1], obj2[:, 2], obj2[:, 3]
        width_obj1, height_obj1 = obj1_x2 - obj1_x1, obj1_y2 - obj1_y1
        width_obj2, height_obj2 = obj2_x2 - obj2_x1, obj2_y2 - obj2_y1
        relative_spat = torch.cat((
            ((obj1_x1 - obj2_x1) / width_obj2)[:, None],
            ((obj1_y1 - obj2_y1) / height_obj2)[:, None],
            ((obj1_x2 - obj2_x2) / width_obj2)[:, None],
            ((obj1_y2 - obj2_y2) / height_obj2)[:, None],
            torch.log(width_obj1/width_obj2)[:, None],
            torch.log(height_obj1*height_obj2)[:, None],
            ), 1)
        return relative_spat
    ...
    def forward(self, fmaps, obj_logits, im_inds, rel_inds, msg_rel_inds, reward_rel_inds, im_sizes, boxes_priors=None, \
        ...
        for iter_i in range(self.num_iter):
            ...
            timestep_obj_input = torch.mm(oo_alpha, timestep_obj_input[:, :self.pooling_dim])
            # timestep_obj_input = torch.mm(oo_alpha, self.W_o(previous_state)) # old version
        ...
        # follow An Interpretable Model for Scene Graph Generation
        sub_spat = rois[:, 1:][rel_inds[:, 1]]
        obj_spat = rois[:, 1:][rel_inds[:, 2]]
        pred_spat = torch.cat((
            torch.min(rois[:, 1:3][rel_inds[:, 1]], rois[:, 1:3][rel_inds[:, 2]]),
            torch.max(rois[:, 3:5][rel_inds[:, 1]], rois[:, 3:5][rel_inds[:, 2]]),),1)
        width_sub, height_sub = sub_spat[:, 2] - sub_spat[:, 0], sub_spat[:, 3] - sub_spat[:, 1]
        width_obj, height_obj = obj_spat[:, 2] - obj_spat[:, 0], obj_spat[:, 3] - obj_spat[:, 1]
        
        so_spat = self.relative_spatial_encode(sub_spat, obj_spat)
        sp_spat = self.relative_spatial_encode(sub_spat, pred_spat)
        po_spat = self.relative_spatial_encode(pred_spat, obj_spat)

        img_area = Variable(torch.from_numpy(im_sizes[:, 0] * im_sizes[:, 1]).cuda())
        sub_encode = torch.cat(((sub_spat[:, 0]/width_sub)[:, None], (sub_spat[:, 1]/height_sub)[:, None],
                                (sub_spat[:, 2]/width_sub)[:, None], (sub_spat[:, 3]/height_sub)[:, None],
                                ((width_sub*height_sub)/(img_area[rel_inds[:, 0]].float()))[:, None]), 1)
        obj_encode = torch.cat(((obj_spat[:, 0]/width_obj)[:, None], (obj_spat[:, 1]/height_obj)[:, None],
                                (obj_spat[:, 2]/width_obj)[:, None], (obj_spat[:, 3]/height_obj)[:, None],
                                ((width_obj*height_obj)/(img_area[rel_inds[:, 0]].float()))[:, None]), 1)
        spat_encode = torch.cat((so_spat, sp_spat, po_spat, sub_encode, obj_encode), 1)
        spat_feat = self.spat2feat(spat_encode)


        triple_rep = torch.cat((triple_rep, spat_feat), 1)
