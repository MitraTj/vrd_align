...
from config import ANCHOR_SIZE, ANCHOR_RATIOS, ANCHOR_SCALES, ModelConfig
...
class Result(object):
    ...
    def __init__(self, od_obj_dists=None, rm_obj_dists=None,
                 ...
                 im_inds=None, fmap=None, rel_dists=None, rel_inds=None, rel_rep=None, rm_gt_boxes=None):
    ...
    def __init__(self, classes, mode='rpntrain', num_gpus=1, nms_filter_duplicates=True,
        ...
        if not self.use_resnet:
            ...
            if conf.rl_offdropout:
                ...
                del self.roi_fmap._modules['5']
                del self.roi_fmap._modules['2']
...
=            self.compress(features) if self.use_resnet else features, rois)    ##feature_pool,shape = [1536, 512, 7, 7]
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        if (not self.training and not self.mode == 'gtbox') or self.mode in ('proposals', 'refinerels'):
            ...
            if (self.training and not self.mode == 'gtbox') or conf.store_det_res:
                ...
                # extra add bbox gt
                rm_gt_boxes = gt_boxes[argmax_overlaps]
            else:
                rm_obj_labels = None
                rm_gt_boxes = None
        else:
