...
##            import pdb; pdb.set_trace()
            ########
            self.k = 5
           # presence_vector = torch.sum(SO_fmaps_extend ** 2, dim=0).view(-1)  # The presence vector is the L2 norm of the embedding at each spatial location.
            SO_fmaps_extend = SO_fmaps_extend.view(num_rels,self.reduce_dim*2, pooling_size_sq, pooling_size_sq) ##[506, 512, 25, 25]
            presence_vector = torch.sum(SO_fmaps_extend ** 2, dim=1)   #[506, 25, 25]
           #SO_fmaps_logits1 = SO_fmaps_logits.view(506,-1)
           #  presence_vector = torch.sum(SO_fmaps_logits1 ** 2, dim=1)
            topk_idxes = torch.topk(presence_vector.view(-1), k=self.k)[1]
           # m_vector = torch.sum(SO_fmaps_extend, dim=0).view(-1)
            m_vector = torch.sum(SO_fmaps_extend, dim=1)    #[506, 25,25]
            latent_mask = torch.zeros_like(presence_vector.view(-1))
            latent_mask[topk_idxes] = 1
            attended_vector = m_vector.view(-1) * latent_mask  # zeros out the activations at masked spatial locations
           ...
           # class_preds = F.log_softmax(self.fc1(attended_vector).unsqueeze(dim=0), dim=1)
            SO_fmaps_scores = F.log_softmax(attended_vector.unsqueeze(dim=0), dim=1)
            SO_fmaps_scores =  SO_fmaps_scores.view(num_rels, pooling_size_sq, pooling_size_sq)
