...
def filter_dets_np(boxes, obj_scores, obj_classes, rel_inds, pred_scores):
    if len(boxes.shape) != 2:
        raise ValueError("Boxes needs to be [num_box, 4] but its {}".format(boxes.size()))

    num_box = boxes.shape[0]
    assert obj_scores.shape[0] == num_box

    assert obj_classes.shape == obj_scores.shape
    num_rel = rel_inds.shape[0]
    assert rel_inds.shape[1] == 2
    assert pred_scores.shape[0] == num_rel

    obj_scores0 = obj_scores[rel_inds[:,0]]
    obj_scores1 = obj_scores[rel_inds[:,1]]

    pred_scores_max = np.max(pred_scores[:, 1:], axis=1)
    pred_classes_argmax = np.argmax(pred_scores[:,1:], axis=1)
    pred_classes_argmax = pred_classes_argmax + 1
