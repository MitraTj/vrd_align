...
from config import RELS_PER_IMG_SGCLS, REL_FG_FRACTION
...
def proposal_assignments_gtbox(rois, gt_boxes, gt_classes, gt_rels, image_offset, fg_thresh=0.5):
    ...
    """
    im_inds = rois[:,0].long()
    rels_per_img = RELS_PER_IMG_SGCLS
    ...
    if num_bg > 0:
        if fg_rels.shape[1] - 3 == 1:
            ...
            bg_rels = torch.cat((
                im_inds[is_bgcand[:, 0]][:, None],
                is_bgcand,
                torch.ones((is_bgcand.shape[0], 1)).long().cuda(is_bgcand.get_device()),
                torch.zeros((is_bgcand.shape[0], fg_rels.shape[1] - 4)).long().cuda(is_bgcand.get_device())
                ), 1)
