...
from config import VG_IMAGES, IM_DATA_FN, VG_SGG_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN, ModelConfig
...
class VG(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for k, v in all_rel_sets.items():
            v = list(set(v))
            if self.filter_duplicate_rels and len(v) > 1:
                v = [np.random.choice(v)]

            gt_rels_pred = np.zeros((self.num_predicates,))
            gt_rels_pred[v] = 1

            gt_rels_subobj.append([k[0], k[1]])
            gt_rels_pred_list.append(gt_rels_pred)

        gt_rels_subobj = np.array(gt_rels_subobj)
        gt_rels_pred_list = np.array(gt_rels_pred_list)
        ...
        entry = {
            ...
            'gt_relations': np.concatenate((gt_rels_subobj, gt_rels_pred_list), axis=1),
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    for i in range(len(image_index)):
        ...
        if conf.filte_large and (mode == 'train'):
            if boxes_i.shape[0] > 25:
...
def dist2idx(rel_dists):
    rel_idx, rel_type = rel_dists[:, 2:].nonzero()
    return np.concatenate((rel_dists[:, :2][rel_idx], rel_type[:, None]), 1)

def vg_collate(data, num_gpus=3, is_train=False, mode='det', return_fn=False):
    assert mode in ('det', 'rel')
    blob = Blob(mode=mode, is_train=is_train, num_gpus=num_gpus,
                batch_size_per_gpu=len(data) // num_gpus, return_fn=return_fn)
    for d in data:
        blob.append(d)
    blob.reduce()
    return blob
...
class VGDataLoader(torch.utils.data.DataLoader):
    ...
    @classmethod
    def splits(cls, train_data, val_data, batch_size=3, num_workers=1, num_gpus=3, mode='det', return_fn=False,
               **kwargs):
        assert mode in ('det', 'rel')
        train_load = cls(
            dataset=train_data,
            ...
            collate_fn=lambda x: vg_collate(x, mode=mode, num_gpus=num_gpus, is_train=True, return_fn=return_fn),
            drop_last=True,
            pin_memory=True,
            **kwargs,
        )
        val_load = cls(
            ...
            shuffle=False,
            num_workers=num_workers,
            collate_fn=lambda x: vg_collate(x, mode=mode, num_gpus=num_gpus, is_train=False, return_fn=return_fn),
            drop_last=True,
            pin_memory=True,
            **kwargs,
        )
