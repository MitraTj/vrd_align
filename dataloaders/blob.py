...
class Blob(object):
    def __init__(self, mode='det', is_train=False, num_gpus=1, primary_gpu=0, batch_size_per_gpu=3, return_fn=False):
        """
    ...
    def __getitem__(self, index):
        ...
        # h, w, scale = self.im_sizes[0, index]
        # train_anchors_, train_anchor_inds_, train_anchor_targets_, train_anchor_labels_ = \
        #         anchor_target_layer(self.gt_boxes[(self.gt_classes[:, 0] == index).nonzero()[:, 0]].data.cpu().numpy(), (h, w))
        ...
        if self.return_fn:
            if index == 0 and self.num_gpus == 1:
                ...
                if self.is_train:
                    return (self.imgs, self.im_sizes[0], image_offset,
                            self.gt_boxes, self.gt_classes, rels, proposals, self.train_anchor_inds, self.fn_list)
                return self.imgs, self.im_sizes[0], image_offset, self.gt_boxes, self.gt_classes, rels, proposals, self.fn_list
            ...
            if self.is_train:
                ...
                self.gt_boxes[index], self.gt_classes[index], rels_i, None, self.train_anchor_inds[index], self.fn_list)
            return (self.imgs[index], self.im_sizes[index], image_offset,
                    self.gt_boxes[index], self.gt_classes[index], rels_i, None, self.fn_list)
